...
/**
 * AJAX endpoint (?action=moves): returns data.json's whole "moves" table
 * (331 PvPoke moves keyed by normalized display name), fetched once by
 * script.js on page load to power move-detail tooltips over every fast/
 * charged move name shown in the league and raid-attacker tables.
 */
function handle_moves_request(): void
{
    header('Content-Type: application/json; charset=utf-8');
    ini_set('display_errors', '0');

    try {
        echo json_encode(['success' => true, 'moves' => load_game_data()['moves']]);
    } catch (\Throwable $e) {
        error_log('Pokecheck moves failed: ' . $e->getMessage());
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => 'Could not load move data.']);
    }
}
...
<style>
  ...
  * {
  ...
  /* Move name -> tooltip trigger. Looks like plain text (not an obvious
     button) since most move names in the tables are just this, with a
     dotted underline as the only affordance that it's tappable. */
  .move-chip {
    background: none;
    border: none;
    padding: 0;
  ...
  /* Hover (desktop) / tap (mobile) on any small Pokemon image - the hero
     thumbnail, a family-strip icon, or a Top Counters attacker icon -
     shows the same species' full "hero" artwork (260x260, already the
     largest asset this app ships - see images/README.md) in a floating
     preview, so an unfamiliar name in Top Counters (e.g. "Dusk Mane
     Necrozma") can actually be looked at rather than squinted at as a
     40px icon. See openImageZoom()/positionFloatingPopover() in
     script.js - shares its clamped-to-viewport positioning with the move
     tooltip above, just as a separate popover instance. */
  .zoomable-img { cursor: zoom-in; }

  .zoomable-img.zoom-active {
    outline: 2px solid var(--accent);
    outline-offset: 2px;
  }
