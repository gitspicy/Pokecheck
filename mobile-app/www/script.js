/**
 * Pokecheck front-end controller - standalone/APK build.
 *
 * This is a copy of the web app's script.js with one deliberate
 * difference: the calls that hit index.php over AJAX instead call
 * PvPEngine (pvp-engine.js), a client-side port of the same PHP logic
 * reading the same data.json/rankings CSVs bundled alongside this page.
 * There is no server and no network access at runtime - see localAction()
 * below, which wraps a synchronous PvPEngine call in the same jQuery
 * Deferred interface ($.ajax would have returned, so the rest of this
 * file's .done()/.fail()/.always() chains are untouched. The Settings
 * panel's update-checker (checkForUpdates()) is the one deliberate
 * exception - it needs to reach the real internet, so its $.ajax calls
 * are left as real network requests in this file, unlike everywhere else.
 */
(function ($) {
  'use strict';

  /**
   * Runs a PvPEngine call once engine data is loaded, resolving/rejecting
   * a jQuery Deferred the same way $.ajax's promise would - so callers
   * keep using .done()/.fail()/.always() unchanged. `worker` returns a
   * plain result object; a "success:false" result rejects (mirroring the
   * old endpoint's non-2xx + JSON-body error convention), matched against
   * jqXHR.responseJSON the same way the old .fail() handler read it.
   */
  function localAction(worker) {
    var deferred = $.Deferred();

    PvPEngine.init().then(function () {
      try {
        var result = worker();
        if (result && result.success === false) {
          deferred.reject({ responseJSON: result });
        } else {
          deferred.resolve(result);
        }
      } catch (e) {
        deferred.reject({ responseJSON: { success: false, error: 'Unexpected error: ' + e.message } });
      }
    }, function (loadError) {
      deferred.reject({
        responseJSON: {
          success: false,
          error: 'Could not load Pokecheck\'s local data. Try reinstalling the app.',
        },
      });
      console.error('PvPEngine failed to load its data:', loadError);
    });
  ...
  function loadSpeciesList() {
    localAction(function () { return PvPEngine.speciesList(); })
      .done(function (data) {
  ...
  function loadMovesTable() {
    localAction(function () { return PvPEngine.moves(); })
      .done(function (data) {
  ...
  function loadTypeChart() {
    localAction(function () { return PvPEngine.typeChart(); })
      .done(function (data) {
  ...
  function loadLeaderboard(leagueId) {
    ...
    return localAction(function () { return PvPEngine.leaderboard(leagueId); })
      .done(function (data) {
  ...
  function performSearch() {
    ...
    localAction(function () { return PvPEngine.search(term); })
      .done(handleResult)
      .fail(function (jqXHR) {
